import { fsaModules } from '../data/financial-statement-analysis';
...
const SEED_ATTEMPT_ID = 'fsa-attempt-1';

interface ModuleScoreConfig {
  moduleKey: keyof typeof fsaModules;
  moduleId: string;
  score: number;
  total: number;
}
...
const moduleScores: ModuleScoreConfig[] = [
  { moduleKey: 'module1', moduleId: 'fsa-m1', score: 29, total: 34 },
  { moduleKey: 'module2', moduleId: 'fsa-m2', score: 33, total: 52 },
  { moduleKey: 'module3', moduleId: 'fsa-m3', score: 12, total: 21 },
  { moduleKey: 'module4', moduleId: 'fsa-m4', score: 33, total: 40 },
  { moduleKey: 'module5', moduleId: 'fsa-m5', score: 8, total: 15 },
  { moduleKey: 'module6', moduleId: 'fsa-m6', score: 39, total: 63 },
  { moduleKey: 'module7', moduleId: 'fsa-m7', score: 40, total: 53 },
  { moduleKey: 'module8', moduleId: 'fsa-m8', score: 16, total: 27 },
  { moduleKey: 'module9', moduleId: 'fsa-m9', score: 14, total: 18 },
  { moduleKey: 'module12', moduleId: 'fsa-m12', score: 17, total: 28 },
];
...
function generateModuleAttempts(
  ...
  const module = fsaModules[moduleKey];
...
/**
 * Seeds a Financial Statement Analysis practice attempt with the user's actual scores.
 * Checks if already seeded before creating to prevent duplicates.
 */
export function seedFsaAttempt(): PracticeAttempt {
  const existing = getAttempts('Financial Statement Analysis');
  ...
  const moduleResults: ModuleResult[] = moduleScores.map(config => {
    ...
    return {
      ...
      moduleName: fsaModules[config.moduleKey].name,
      score: config.score,
      total: config.total,
      percentage,
  ...
  const attempt: PracticeAttempt = {
    ...
    attemptNumber: 1,
    completedAt: new Date('2025-01-18T10:15:00Z').toISOString(),
    moduleResults,
    overallScore,
    overallTotal,
    overallPercentage,
